...
static int innodb_init_params()
{
	...
	if (srv_flush_log_at_trx_commit == 2) {
		/* Compatibility with old releases for Windows:
		Enable the file system cache if
		innodb_flush_log_at_trx_commit=2 at startup. */
...
static MYSQL_SYSVAR_BOOL(log_file_buffering, log_sys.log_buffered,
  PLUGIN_VAR_NOCMDARG,
  "Enable the file system cache for the redo log.",
